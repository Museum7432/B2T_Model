...
class B2T_Dataset(Dataset):
    ...
    def _get_one(self, idx):
        ...
        if self.add_noises:
            ...
            noise = np.random.normal(loc=0.0, scale=0.04, size=256).astype(
                spikePow.dtype
            )
    ...
    def __getitem__(self, idx):
        ...
        # temp = phonemizedText.replace('|', ',|,').replace('-', ',').replace('+', ',').split(',')
        # temp  = ['|' if t == '|' else 'e' for t in temp]
        # temp = "".join(temp)

        temp = sentenceText.replace('|', " | ").split()
        temp  = ['|' if t == '|' else 'e' for t in temp]
        temp = "".join(temp)
        ...
        re["WS"] = temp.replace("e", "w")
        re["WS_ids"] = tk
        re["WS_ids_lens"] = len(tk)
...
def collate_fn_factory(add_noises=False):
    ...
    fields_to_pad = ["sent_ids", "phonemize_ids", "WS_ids"]
