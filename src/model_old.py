...
from .modules.others import Pack, UnPack, unpack
...
class joint_Model(BaseModel):
    ...
    def forward(self, spikePow, spikePow_mask, spikePow_lens, encoder_only=False):
        # unpack the spikePow
        cu_seqlens = F.pad(spikePow_lens.cumsum(0), pad=(1, 0), value=0).to(torch.int32)

        spikePow = unpack(spikePow, cu_seqlens)
