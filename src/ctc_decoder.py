...
def _get_word_dict(lexicon, lm, lm_dict, tokens_dict, unk_word):
    ...
    if lexicon and word_dict is None:
        ...
        d = {
            tokens_dict.get_entry(i): [[tokens_dict.get_entry(i)]]
            for i in range(tokens_dict.index_size())
        }
        d[unk_word] = [[unk_word]]
...
class CTCDecoderLM(_LM):
    ...
    @abstractmethod
    def score(
        self, state: CTCDecoderLMState, usr_token_idx: int
...
class CTCDecoder:
    ...
    def __init__(
        ...
        if lexicon:
            ...
            self.decoder = _LexiconFreeDecoder(
                decoder_options, lm, silence, self.blank, transitions
            )
...
class CustomLM(CTCDecoderLM):
    ...
    def __init__(self, model_ids="gpt2", device="cuda", word_dict=None):
        CTCDecoderLM.__init__(self)
        # self.language_model = language_model

        self.word_dict = word_dict

        # information assosiate with each state
        self.infos = {}
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_ids)
        model = GPT2LMHeadModel.from_pretrained(model_ids)
        model.eval()
        model = model.to(device)

        self.model = model

    def past_key_values_to_device(self, past_key_values, device=None):
        if device is None:
            device = self.device
        ...
        for i in range(len(past_key_values)):
            ...
            for j in range(len(past_key_values[i])):
                past_key_values[i][j] = past_key_values[i][j].to(device)

        return past_key_values

    def calc_score(self, input_ids, last_token_logit, past_key_values=None):
        seq_len = len(input_ids)
        # last_token_logit (vocab_size): the logit of the token that
        # predict the first token of input_ids
        ...
        # (vocab_size)
        last_token_logit = last_token_logit.to(self.device)

        if past_key_values:
            past_key_values = self.past_key_values_to_device(past_key_values)

        with torch.no_grad():
            # seq_len, vocab_size
            output = self.model(input_ids, past_key_values=past_key_values)

        new_last_token_logit = output.logits[-1]

        # seq_len
        # this predict the input_ids
        lm_logits = torch.concat([last_token_logit.unsqueeze(0), output.logits[:-1]])

        # lm_logits = torch.softmax(lm_logits, dim=-1)
        # score = lm_logits.gather(1, input_ids.unsqueeze(1)).squeeze(1)
        # score = score.mean()

        loss_fct = torch.nn.CrossEntropyLoss(reduction="mean")
        score = 1/(loss_fct(lm_logits, input_ids) + 0.001)

        new_past_key_values = output.past_key_values

        new_past_key_values = self.past_key_values_to_device(
            new_past_key_values, device="cpu"
        )

        new_last_token_logit = new_last_token_logit.cpu()


        return score.cpu(), new_past_key_values, new_last_token_logit

    def start(self, start_with_nothing: bool) -> CTCDecoderLMState:
        state = CTCDecoderLMState()

        tokenized = self.tokenizer.encode(
            self.tokenizer.bos_token, return_tensors="pt"
        )[0].to(self.device)

        with torch.no_grad():
            output = self.model(tokenized, past_key_values=None)

        new_last_token_logit = output.logits[-1].cpu()
        new_past_key_values = self.past_key_values_to_device(
            output.past_key_values, device="cpu"
        )

        self.infos[state] = {
            "score": 0,
            "last_token_logit": new_last_token_logit,
            "past_key_values": new_past_key_values,
            "last_token": self.tokenizer.bos_token
        }
    ...
    def score(
        ...
        word = self.word_dict.get_entry(usr_token_idx)
        return self._score_word(state, word, usr_token_idx)

    def _score_word(
        self, state: CTCDecoderLMState, word: str, usr_token_idx: int
    ) -> Tuple[CTCDecoderLMState, float]:
        ...
        current_state_info = self.infos[state]

        # create input_ids from word
        word = " " + word
        input_ids = self.tokenizer.encode(word, return_tensors="pt")[0]

        new_score, new_past_key_values, new_last_token_logit = self.calc_score(
            input_ids=input_ids,
            last_token_logit=current_state_info["last_token_logit"],
            past_key_values=current_state_info["past_key_values"],
        )
        # then save the last token logit (that predict the first token of word)

        # if current_state_info["last_token"] == self.tokenizer.bos_token:
        #     new_score = 0
    ...
    def finish(self, state: CTCDecoderLMState) -> Tuple[CTCDecoderLMState, float]:
        return self._score_word(state, self.tokenizer.eos_token, -1)
...
def lm_ctc_decoder(
    ...
    word_dict = _get_word_dict(lexicon, None, None, tokens_dict, unk_word)

    lm = CustomLM(model_ids=model_ids, device=device, word_dict=word_dict)
